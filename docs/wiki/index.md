...
- [multi-project](architecture/multi-project.md) — one daemon instance managing many repos, Symphony's `WORKFLOW.md` pattern adapted as repo-owned `lucid.project.toml` per-project config (shipped — `[[projects]]` + `Config::validate_projects()`), directory-detected `--project` CLI flag across the `task` subcommands (shipped — real overrides on `dispatch-now`, flag-acceptance-and-validation only on `list`/`create`/`approve`/`reject`)
