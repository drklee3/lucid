...
4. Relax `daemon.rs`'s sequential loop for `Sandboxed` dispatches specifically. Not built yet — today's dispatch loop is still sequential regardless of backend.
