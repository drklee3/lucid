...
`ProjectConfig` (`src/config.rs`) carries the ticket-source concept as `ticket_source: TicketSource`, `#[serde(default)]` so an existing `lucid.project.toml` with no `ticket_source` line keeps loading as `OperatorOnly` — the trust-neutral default (operator's own CLI or direct tracker approval, same trust level as running lucid locally). Setting `ticket_source = "accepts-external"` (kebab-case, matching every other enum field in this config) opts a project into allowing tracker-native external ticket creation (e.g. a teammate's Discord message — see [human-in-the-loop](human-in-the-loop.md)).

`Config::validate_trust_routing(&self, projects: &[ProjectConfig])` checks whether any configured `harness_profiles` entry has `execution_backend == ExecutionBackend::Sandboxed`; if at least one does, every project passes regardless of its `ticket_source`. Otherwise, it walks `self.projects` zipped against the resolved `projects` (the `validate_projects()` output, same order), and bails with an error naming the offending project's path for the first `AcceptsExternal` project it finds. `OperatorOnly` projects are exempt in either case — this rail targets external-trigger risk specifically, not a blanket sandboxing mandate. `config_validate` (`src/main.rs`) calls it right after `validate_projects()`, so a config with an `AcceptsExternal` project and no `Sandboxed` profile fails `lucid config validate` outright rather than only failing at dispatch time.
...
3. **Done.** `lucid config validate` trust-routing rail. `ProjectConfig.ticket_source: TicketSource` (`OperatorOnly` default, `AcceptsExternal` opt-in) plus `Config::validate_trust_routing` — see "Trust routing" above for the shipped shape.
