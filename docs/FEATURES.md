...
- [x] Decision surface: for `LinearAdapter`, moves the issue's real ticket `state` field (`Pending`/`Approved`/`In Review`/`Done`/`Rejected`/`Stale`, six named workflow states required per team) via `issueUpdate`, not a label — see `docs/wiki/architecture/tracker-adapter.md` § Decision state: the issue's real ticket state, not a label. `FileTracker` filters directly on its own `decision_state` field.
