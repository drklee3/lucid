...
- **After implementing or making code changes, launch a background subagent to update the wiki** — `Agent` with `subagent_type: "fork"` (not a fresh general-purpose agent: a fork inherits the whole conversation, so it knows *why* something changed, not just the diff). Have it apply this page's rules to what just happened: new/changed pages, `index.md`, an appended `log.md` entry, `docs/FEATURES.md` checkboxes, and a lint pass if the change was large. Do this for any change that needs more than a one-line commit message to explain — trivial mechanical edits (typo fixes, formatting, a bumped dependency version) don't need it. Launch it as the last step before ending the turn, not blocking on it — the point is the main thread doesn't have to do the doc bookkeeping inline the way this instruction was itself added.
